...
const ProjectsView = ({ projects, setProjects }) => {
  ...
  const addUpdateCallback = (result, forUpdate) => {
    if (forUpdate)
      return setProjects((prev) => ({
        ...prev,
        records: prev.records.map((i) => (i.id === result.id ? result : i)),
      }));
    setProjects((prev) => ({ ...prev, records: [result, ...prev.records] }));
  };

  if (projects.loading) return <div>loading...</div>;

  if (projects.records.length < 1) {
    return (
  ...
  return (
    <div className="space-y-5">
      ...
      {pageView === "list" && (
        <ProjectList data={projects.records} table={table} columns={columns} />
      )}
